/*
 * Port of fs2.pdf.codec.Newline to Scala 3 + scodec 2.3.
 */

package zio.pdf.codec

import _root_.scodec.{Codec, Decoder}
import _root_.scodec.bits.{BitVector, ByteVector}
import _root_.scodec.bits.hex
import _root_.scodec.codecs.*

private[pdf] object Newline {

  val lfBytes:    ByteVector = hex"0A"
  val lfBits:     BitVector  = lfBytes.bits
  val lfByte:     Byte       = lfBytes.toByte()

  val crBytes:    ByteVector = hex"0D"
  val crBits:     BitVector  = crBytes.bits
  val crByte:     Byte       = crBytes.toByte()

  val crlfBytes:  ByteVector = hex"0D0A"
  val crlfBits:   BitVector  = crlfBytes.bits

  val spaceBytes: ByteVector = hex"20"
  val spaceByte:  Byte       = spaceBytes.toByte()

  val tabBytes:   ByteVector = hex"0B"

  val newlines:   List[ByteVector] = List(lfBytes, crlfBytes, crBytes)

  val lf:   Codec[Unit] = constant(lfBytes)
  val cr:   Codec[Unit] = constant(crBytes)
  val crlf: Codec[Unit] = constant(crlfBytes)

  /** Match LF, CR-LF, or CR (in that order). */
