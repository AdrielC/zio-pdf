...
/** internal Gitea Package Registry — internal Maven only (no Sonatype / public mirror). */
object PublishSettings {

  private def gitServer   = sys.env.getOrElse("GIT_SERVER", "git.internal.net")
  ...
  val settings: Seq[Def.Setting[?]] = Seq(
    ...
    publishTo := {
      val mavenPath = "/api/packages/internal/maven"
  ...
  /** Resolver for other internal projects consuming this artifact. */
  val internalResolver: Resolver =
    "internal Gitea Maven" at s"$gitProtocol://$gitServer/api/packages/internal/maven"
}
