...
/**
 * `Pure[W, S1, S2, R, E, A]` is a purely functional description of a
 ...
 * polymorphism `Pure` can be used to model a variety of effects including
 * context, state, failure, and logging.
 */

sealed abstract class Pure[+W, -S1, +S2, -R, +E, +A] { self =>
  import Pure._

  /**
   * A symbolic alias for `zipRight`.
   */
  final def *>[W1 >: W, S3, R1 <: R, E1 >: E, B](that: Pure[W1, S2, S3, R1, E1, B]): Pure[W1, S1, S3, R1, E1, B] =
    self.zipRight(that)

  /**
   * A symbolic alias for `zipLeft`.
   */
  final def <*[W1 >: W, S3, R1 <: R, E1 >: E, B](that: Pure[W1, S2, S3, R1, E1, B]): Pure[W1, S1, S3, R1, E1, A] =
    self.zipLeft(that)

  /**
   * A symbolic alias for `zip`.
   */
  final def <*>[W1 >: W, S3, R1 <: R, E1 >: E, B](that: Pure[W1, S2, S3, R1, E1, B])(implicit
    zippable: Zippable[A, B]
  ): Pure[W1, S1, S3, R1, E1, zippable.Out] =
    self.zip(that)
  ...
  final def <+>[W1 >: W, S0 <: S1, S3 >: S2, R1 <: R, E1, B](
    that: => Pure[W1, S0, S3, R1, E1, B]
  )(implicit ev: CanFail[E]): Pure[W1, S0, S3, R1, E1, Either[A, B]] =
  ...
  final def <>[W1 >: W, S0 <: S1, S3 >: S2, R1 <: R, E1, A1 >: A](
    that: => Pure[W1, S0, S3, R1, E1, A1]
  )(implicit ev: CanFail[E]): Pure[W1, S0, S3, R1, E1, A1] =
    self orElse that

  /**
   * A symbolic alias for `flatMap`.
   */
  final def >>=[W1 >: W, S3, R1 <: R, E1 >: E, B](f: A => Pure[W1, S2, S3, R1, E1, B]): Pure[W1, S1, S3, R1, E1, B] =
    self.flatMap(f)

  /**
   * A symbolic alias for `log`.
   */
  final def ??[W1 >: W](w: W1): Pure[W1, S1, S2, R, E, A] =
    self.log(w)
   ...
   * computation.
   */
  final def absolve[E1 >: E, B](implicit ev: A <:< Either[E1, B]): Pure[W, S1, S2, R, E1, B] =
    flatMap(ev(_).fold(fail, succeed))

  /**
   * Maps the success value of this computation to a constant value.
   */
  final def as[B](b: => B): Pure[W, S1, S2, R, E, B] =
    map(_ => b)

  /**
   * Maps the success value of this computation to the optional value.
   */
  final def asSome: Pure[W, S1, S2, R, E, Option[A]] =
    map(Some(_))

  /**
   * Maps the error value of this computation to the optional value.
   */
  final def asSomeError(implicit ev: CanFail[E]): Pure[W, S1, S2, R, Option[E], A] =
    mapError(Some(_))

  /**
   * Maps the output state to a constant value
   */
  final def asState[S3](s: S3): Pure[W, S1, S3, R, E, A] =
    mapState(_ => s)
   ...
   */
  final def bimap[E1, B](f: E => E1, g: A => B)(implicit ev: CanFail[E]): Pure[W, S1, S2, R, E1, B] =
  ...
  final def catchSome[W1 >: W, S0 <: S1, S3 >: S2, R1 <: R, E1 >: E, A1 >: A](
    pf: PartialFunction[E, Pure[W1, S0, S3, R1, E1, A1]]
  )(implicit ev: CanFail[E]): Pure[W1, S0, S3, R1, E1, A1] =
    catchAll(pf.applyOrElse[E, Pure[W1, S0, S3, R1, E1, A1]](_, fail))
   ...
   */
  final def clearLogOnError: Pure[W, S1, S2, R, E, A] =
   ...
   */
  final def collect[E1 >: E, B](e: => E1)(pf: PartialFunction[A, B]): Pure[W, S1, S2, R, E1, B] =
  ...
  final def collectM[W1 >: W, S3, R1 <: R, E1 >: E, B](
    e: => E1
  )(pf: PartialFunction[A, Pure[W1, S2, S3, R1, E1, B]]): Pure[W1, S1, S3, R1, E1, B] =
    flatMap(pf.applyOrElse[A, Pure[W1, S2, S3, R1, E1, B]](_, _ => fail(e)))

  /**
   * Transforms the initial state of this computation with the specified
   * function.
   */
  final def contramapState[S0](f: S0 => S1): Pure[W, S0, S2, R, E, A] =
    update(f) *> self
   ...
   */
  final def either[S3 >: S2 <: S1](implicit ev: CanFail[E]): Pure[W, S3, S3, R, Nothing, Either[E, A]] =
    fold(Left(_), Right(_))
  ...
  final def filterOrElse[W1 >: W, S3 >: S2, R1 <: R, E1 >: E, A1 >: A](
    p: A => Boolean
  )(f: A => Pure[W1, S2, S3, R1, E1, A1]): Pure[W1, S1, S3, R1, E1, A1] =
    self.flatMap {
      case v if !p(v) => f(v)
      case v          => Pure.succeed(v)
    }

  /**
   * Similar to `filterOrElse`, but instead of a function it accepts the Pure computation
   * to apply if the predicate fails.
   */
  final def filterOrElse_[W1 >: W, S3 >: S2, R1 <: R, E1 >: E, A1 >: A](p: A => Boolean)(
    zPure: => Pure[W1, S2, S3, R1, E1, A1]
  ): Pure[W1, S1, S3, R1, E1, A1] =
    filterOrElse[W1, S3, R1, E1, A1](p)(_ => zPure)

  /**
   * Fails with the specified error if the predicate fails.
   */
  final def filterOrFail[E1 >: E](p: A => Boolean)(e: => E1): Pure[W, S1, S2, R, E1, A] =
    filterOrElse_[W, S2, R, E1, A](p)(Pure.fail(e))
  ...
  final def flatMap[W1 >: W, S3, R1 <: R, E1 >: E, B](
    f: A => Pure[W1, S2, S3, R1, E1, B]
  ): Pure[W1, S1, S3, R1, E1, B] =
    FlatMap(self, f)
   ...
   */
  final def flip[S3 >: S2 <: S1]: Pure[W, S3, S3, R, A, E] =
    foldM(succeed, fail)
  ...
  final def fold[S3 >: S2 <: S1, B](failure: E => B, success: A => B)(implicit
    ev: CanFail[E]
  ): Pure[W, S3, S3, R, Nothing, B] =
    self.foldM(e => Pure.succeed(failure(e)), a => Pure.succeed(success(a)))
  ...
  final def foldM[W1 >: W, S0 <: S1, S3, R1 <: R, E1, B](
    failure: E => Pure[W1, S0, S3, R1, E1, B],
    success: A => Pure[W1, S2, S3, R1, E1, B]
  )(implicit ev: CanFail[E]): Pure[W1, S0, S3, R1, E1, B] =
    Fold(self, failure, success)

  /**
   * Exposes the output state into the value channel.
   */
  final def getState: Pure[W, S1, S2, R, E, (S2, A)] =
   ...
   */
  final def head[B](implicit ev: A <:< List[B]): Pure[W, S1, S2, R, Option[E], B] =
    foldM(
      e => Pure.fail(Some(e)),
      a =>
        ev(a).headOption match {
          case Some(b) => Pure.succeed(b)
          case None    => Pure.fail(None)
        }
    )
   ...
   */
  final def keepLogOnError: Pure[W, S1, S2, R, E, A] =
    ClearLogOnError(value = false, self)

  /**
   * Returns a successful computation if the value is `Left`, or fails with error `None`.
   */
  final def left[B, C](implicit ev: A <:< Either[B, C]): Pure[W, S1, S2, R, Option[E], B] =
    foldM(
      e => Pure.fail(Some(e)),
      a => ev(a).fold(Pure.succeed, _ => Pure.fail(None))
    )

  /**
   * Returns a successful computation if the value is `Left`, or fails with error `e`.
   */
  final def leftOrFail[B, C, E1 >: E](e: => E1)(implicit ev: A <:< Either[B, C]): Pure[W, S1, S2, R, E1, B] =
    flatMap(ev(_) match {
      case Right(_)    => Pure.fail(e)
      case Left(value) => Pure.succeed(value)
    })

  /**
   * Returns a successful computation if the value is `Left`, or fails with the given error function `e`.
   */
  final def leftOrFailWith[B, C, E1 >: E](e: C => E1)(implicit ev: A <:< Either[B, C]): Pure[W, S1, S2, R, E1, B] =
    flatMap(ev(_) match {
      case Right(err)  => Pure.fail(e(err))
      case Left(value) => Pure.succeed(value)
    })
  ...
  final def leftOrFailWithException[B, C, E1 >: NoSuchElementException](implicit
    ...
    foldM(
      e => Pure.fail(ev2(e)),
      a => ev(a).fold(Pure.succeed, _ => Pure.fail(new NoSuchElementException("Either.left.get on Right")))
    )

  final def log[W1 >: W](w: W1): Pure[W1, S1, S2, R, E, A] =
    self.flatMap(a => Pure.log(w).as(a))

  /**
   * Transforms the result of this computation with the specified function.
   */
  final def map[B](f: A => B): Pure[W, S1, S2, R, E, B] =
    FMap(self, f)
   ...
   * function.
   */
  final def mapError[E1](f: E => E1)(implicit ev: CanFail[E]): Pure[W, S1, S2, R, E1, A] =
    catchAll(e => fail(f(e)))
   ...
   * function.
   */
  final def mapState[S3](f: S2 => S3): Pure[W, S1, S3, R, E, A] =
    self <* update(f)

  /**
   * Negates the boolean value of this computation.
   */
  final def negate(implicit ev: A <:< Boolean): Pure[W, S1, S2, R, E, Boolean] =
    map(!_)

  /**
   * Requires the value of this computation to be `None`, otherwise fails with `None`.
   */
  final def none[B](implicit ev: A <:< Option[B]): Pure[W, S1, S2, R, Option[E], Unit] =
    self.foldM(
      e => Pure.fail(Some(e)),
      a => a.fold[Pure[W, S2, S2, R, Option[E], Unit]](Pure.unit)(_ => Pure.fail(None))
    )
   ...
   */
  final infix def orElse[W1 >: W, S0 <: S1, S3 >: S2, R1 <: R, E1, A1 >: A](
    that: => Pure[W1, S0, S3, R1, E1, A1]
  )(implicit ev: CanFail[E]): Pure[W1, S0, S3, R1, E1, A1] =
   ...
   * otherwise executes the specified computation.
   */
  final infix def orElseEither[W1 >: W, S0 <: S1, S3 >: S2, R1 <: R, E1, B](
    that: => Pure[W1, S0, S3, R1, E1, B]
  )(implicit ev: CanFail[E]): Pure[W1, S0, S3, R1, E1, Either[A, B]] =
   ...
   */
  final infix def orElseFail[E1](e1: => E1)(implicit ev: CanFail[E]): Pure[W, S1, S2, R, E1, A] =
    orElse(fail(e1))
   ...
   */
  final infix def orElseOptional[W1 >: W, S0 <: S1, S3 >: S2, R1 <: R, E1, A1 >: A](
    that: => Pure[W1, S0, S3, R1, Option[E1], A1]
  )(implicit ev: E <:< Option[E1]): Pure[W1, S0, S3, R1, Option[E1], A1] =
    catchAll(ev(_).fold(that)(e => Pure.fail(Some(e))))

  /**
   * Executes this computation and returns its value, if it succeeds, but
   * otherwise succeeds with the specified value.
   */
  final def orElseSucceed[A1 >: A](a1: => A1)(implicit ev: CanFail[E]): Pure[W, S1, Any, R, Nothing, A1] =
    orElse(succeed(a1))
   ...
   */
  final def provideState(s: S1): Pure[W, Any, S2, R, E, A] =
    set(s) *> self
  ...
  )(implicit ev1: E <:< Throwable, ev2: CanFail[E]): Pure[W, S1, S2, R, E1, A] =
  ...
  final def refineOrDieWith[E1](pf: PartialFunction[E, E1])(f: E => Throwable)(implicit
    ...
    self.catchAll(err => (pf lift err).fold[Pure[W, S1, S2, R, E1, A]](throw f(err))(Pure.fail))

  /**
   * Fail with the returned value if the `PartialFunction` matches, otherwise
   * continue with our held value.
   */
  final def reject[S0 <: S1, S3 >: S2, R1 <: R, E1 >: E](pf: PartialFunction[A, E1]): Pure[W, S0, S3, R1, E1, A] =
    rejectM(pf.andThen(Pure.fail(_)))
  ...
  final def rejectM[W1 >: W, S0 <: S1, S3 >: S2, R1 <: R, E1 >: E](
    pf: PartialFunction[A, Pure[W1, S2, S3, R1, E1, E1]]
  ): Pure[W1, S0, S3, R1, E1, A] =
    self.flatMap[W1, S3, R1, E1, A] { v =>
      if (pf.isDefinedAt(v)) {
        pf(v).flatMap[W1, S3, R1, E1, A](Pure.fail)
      } else {
        Pure.succeed(v)
      }
    }
   ...
   */
  final def repeatN(n: Int)(implicit ev: S2 <:< S1): Pure[W, S1, S2, R, E, A] =
    self.flatMap(a => if (n <= 0) Pure.succeed(a) else repeatN(n - 1).contramapState(ev))
   ...
   */
  final def repeatUntil(f: A => Boolean)(implicit ev: S2 <:< S1): Pure[W, S1, S2, R, E, A] =
    self.flatMap(a => if (f(a)) Pure.succeed(a) else repeatUntil(f).contramapState(ev))

  /**
   * Repeats this computation until its value is equal to the specified value
   * (or until the first failure) passing the updated state to each successive repetition.
   */
  final def repeatUntilEquals[A1 >: A](a: => A1)(implicit ev: S2 <:< S1): Pure[W, S1, S2, R, E, A1] =
    repeatUntil(_ == a)
   ...
   * (or until the first failure) passing the updated state to each successive repetition.
   */
  final def repeatUntilState(f: S2 => Boolean)(implicit ev: S2 <:< S1): Pure[W, S1, S2, R, E, A] =
    self.zip(Pure.getWith(f)).flatMap { case (a, b) =>
      if (b) Pure.succeed(a)
   ...
   * (or until the first failure) passing the updated state to each successive repetition.
   */
  final def repeatUntilStateEquals[S3 >: S2](s: => S3)(implicit ev: S2 <:< S1): Pure[W, S1, S3, R, E, A] =
   ...
   * (or until the first failure) passing the updated state to each successive repetition.
   */
  final def repeatWhile(f: A => Boolean)(implicit ev: S2 <:< S1): Pure[W, S1, S2, R, E, A] =
    repeatUntil(!f(_))
   ...
   * (or until the first failure) passing the updated state to each successive repetition.
   */
  final def repeatWhileEquals[A1 >: A](a: => A1)(implicit ev: S2 <:< S1): Pure[W, S1, S2, R, E, A1] =
    repeatWhile(_ == a)
   ...
   * (or until the first failure) passing the updated state to each successive repetition.
   */
  final def repeatWhileState(f: S2 => Boolean)(implicit ev: S2 <:< S1): Pure[W, S1, S2, R, E, A] =
   ...
   */
  final def some[B](implicit ev: A <:< Option[B]): Pure[W, S1, S2, R, Option[E], B] =
    self.foldM(
      e => Pure.fail(Some(e)),
      a => a.fold[Pure[W, S2, S2, R, Option[E], B]](Pure.fail(Option.empty))(Pure.succeed)
    )

  /**
   * Extracts the optional value or returns the given 'default' leaving the state unchanged.
   */
  final def someOrElse[B](default: => B)(implicit ev: A <:< Option[B]): Pure[W, S1, S2, R, E, B] =
  ...
  final def someOrElseM[W1 >: W, S3 >: S2, R1 <: R, E1 >: E, B](
    that: Pure[W1, S2, S3, R1, E1, B]
  )(implicit ev: A <:< Option[B]): Pure[W1, S1, S3, R1, E1, B] =
    self.flatMap(ev(_).fold(that)(Pure.succeed))

  /**
   * Extracts the optional value or fails with the given error 'e'.
   */
  final def someOrFail[B, E1 >: E](e: => E1)(implicit ev: A <:< Option[B]): Pure[W, S1, S2, R, E1, B] =
    self.flatMap(ev(_) match {
      case Some(value) => Pure.succeed(value)
      case None        => Pure.fail(e)
    })
  ...
  final def zip[W1 >: W, S3, R1 <: R, E1 >: E, B](
    that: Pure[W1, S2, S3, R1, E1, B]
  )(implicit zippable: Zippable[A, B]): Pure[W1, S1, S3, R1, E1, zippable.Out] =
  ...
  final def zipWith[W1 >: W, S3, R1 <: R, E1 >: E, B, C](
    that: Pure[W1, S2, S3, R1, E1, B]
  )(f: (A, B) => C): Pure[W1, S1, S3, R1, E1, C] =
    self.flatMap(a => that.map(b => f(a, b)))

  /**
   * Returns a successful computation if the value is `Right`, or fails with error `None`.
   */
  final def right[B, C](implicit ev: A <:< Either[B, C]): Pure[W, S1, S2, R, Option[E], C] =
    self.foldM(
      e => Pure.fail(Some(e)),
      a => ev(a).fold(_ => Pure.fail(None), Pure.succeed)
    )

  /*
     Returns a successful computation if the value is `Right`, or fails with error `e`.
   */
  final def rightOrFail[B, C, E1 >: E](e: => E1)(implicit ev: A <:< Either[B, C]): Pure[W, S1, S2, R, E1, C] =
    self.flatMap(ev(_) match {
      case Right(value) => Pure.succeed(value)
      case Left(_)      => Pure.fail(e)
    })

  /*
     Returns a successful computation if the value is `Right`, or fails with error function `e`.
   */
  final def rightOrFailWith[B, C, E1 >: E](e: B => E1)(implicit ev: A <:< Either[B, C]): Pure[W, S1, S2, R, E1, C] =
    self.flatMap(ev(_) match {
      case Right(value) => Pure.succeed(value)
      case Left(err)    => Pure.fail(e(err))
    })
  ...
  final def rightOrFailWithException[B, C, E1 >: NoSuchElementException](implicit
    ...
    self.foldM(
      e => Pure.fail(ev2(e)),
      a => ev(a).fold(_ => Pure.fail(new NoSuchElementException("Either.right.get on Left")), Pure.succeed)
    )

  /**
   * Maps the value of this computation to unit.
   */
  final def unit: Pure[W, S1, S2, R, E, Unit] =
    self.flatMap(succeedUnitFn)

}

object Pure {
  private val succeedUnit: Pure[Nothing, Any, Nothing, Any, Nothing, Unit]            = Succeed(())
  private val succeedNone: Pure[Nothing, Any, Nothing, Any, Nothing, Option[Nothing]] = Succeed(None)
  private val succeedUnitFn                                                            = (_: Any) => succeedUnit

  /**
   * Constructs a computation, catching any `Throwable` that is thrown.
   */
  def attempt[S, A](a: => A): Pure[Nothing, S, S, Any, Throwable, A] =
    suspend {
      try Pure.succeed(a)
      catch {
        case e: VirtualMachineError => throw e
        case e: Throwable           => Pure.fail(e)
      }
    }
  ...
  def collect[W, S, R, E, A, B, Collection[+Element] <: Iterable[Element]](in: Collection[A])(
    f: A => Pure[W, S, S, R, E, Option[B]]
  )(implicit bf: BuildFrom[Collection[A], B, Collection[B]]): Pure[W, S, S, R, E, Collection[B]] =
    if (in.isEmpty) Pure.succeed(bf.fromSpecific(in)(Nil))
    else
      Pure.suspend {
        val iterator = in.iterator
        val builder  = bf.newBuilder(in)

        lazy val recurse: Option[B] => Pure[W, S, S, R, E, Collection[B]] = {
          case Some(b) => builder += b; loop()
          case None    => loop()
        }

        def loop(): Pure[W, S, S, R, E, Collection[B]] =
          if (iterator.hasNext) f(iterator.next()).flatMap(recurse)
          else Pure.succeed(builder.result())
  ...
  def fail[E](e: E): Pure[Nothing, Any, Nothing, Any, E, Nothing] =
    Pure.Fail(e)

  /**
   * Constructs a computation from an `Either`.
   */
  def fromEither[S, L, R](either: Either[L, R]): Pure[Nothing, S, S, Any, L, R] =
    either.fold(l => Pure.fail(l), r => Pure.succeed(r))

  /**
   * Constructs a computation from an `Option`.
   */
  def fromOption[S, A](option: Option[A]): Pure[Nothing, S, S, Any, Unit, A] =
    option match {
      case Some(a) => Pure.succeed(a)
      case None    => Pure.fail(())
    }
   ...
   */
  def fromTry[S, A](t: Try[A]): Pure[Nothing, S, S, Any, Throwable, A] =
    attempt(t).flatMap {
      case scala.util.Success(v) => Pure.succeed(v)
      case scala.util.Failure(t) => Pure.fail(t)
    }
  ...
  def foreach[W, S, R, E, A, B, Collection[+Element] <: Iterable[Element]](in: Collection[A])(
    ...
    if (in.isEmpty) Pure.succeed(bf.fromSpecific(in)(Nil))
      ...
      Pure.suspend {
        ...
        lazy val recurse: B => Pure[W, S, S, R, E, Collection[B]] = { b =>
          builder += b
          loop()
        }
   ...
   */
  def foreachDiscard[W, S, R, E, A, B](in: Iterable[A])(f: A => Pure[W, S, S, R, E, B]): Pure[W, S, S, R, E, Unit] =
    if (in.isEmpty) unit
    else
      Pure.suspend {
        val iterator = in.iterator

        lazy val recurse: Any => Pure[W, S, S, R, E, Unit] = _ => loop()

        def loop(): Pure[W, S, S, R, E, Unit] =
          if (iterator.hasNext) f(iterator.next()).flatMap(recurse)
          else Pure.unit
   ...
   */
  def set[S](s: S): Pure[Nothing, Any, S, Any, Nothing, Unit] =
    update(_ => s)
   ...
   */
  def succeed[S, A](a: A): Pure[Nothing, S, S, Any, Nothing, A] =
    Succeed(a)

  /**
   * Returns a lazily constructed computation, whose construction may itself require effects.
   */
  def suspend[W, S1, S2, R, E, A](pure: => Pure[W, S1, S2, R, E, A]): Pure[W, S1, S2, R, E, A] =
    Pure.unit.flatMap(_ => pure)

  /**
   * Constructs a computation that always returns the `Unit` value, passing the
   * state through unchanged.
   */
  def unit[S]: Pure[Nothing, S, S, Any, Nothing, Unit] =
    succeedUnit

  /**
   * The moral equivalent of `if (!p) exp`
   */
  def unless[W, S, R, E, A](p: Boolean)(pure: => Pure[W, S, S, R, E, A]): Pure[W, S, S, R, E, Option[A]] =
    if (p) none else pure.asSome

  /**
   * Constructs a computation from the specified update function.
   */
  def update[S1, S2](f: S1 => S2): Pure[Nothing, S1, S2, Any, Nothing, Unit] =
    Update(f)

  /**
   * The moral equivalent of `if (p) exp`
   */
  def when[W, S, R, E, A](p: Boolean)(pure: => Pure[W, S, S, R, E, A]): Pure[W, S, S, R, E, Option[A]] =
  ...
  def whenCase[W, S, R, E, A, B](a: A)(
    pf: PartialFunction[A, Pure[W, S, S, R, E, B]]
  ): Pure[W, S, S, R, E, Option[B]] =
  ...
  final class ServiceWithPartiallyApplied[R](private val dummy: Boolean = true) extends AnyVal {
    def apply[S, A](f: R => A)(implicit tag: Tag[R]): Pure[Nothing, S, S, R, Nothing, A] =
  ...
  final class ServiceWithPurePartiallyApplied[R](private val dummy: Boolean = true) extends AnyVal {
    def apply[W, S1, S2, E, A](
      f: R => Pure[W, S1, S2, Any, E, A]
    )(implicit tag: Tag[R]): Pure[W, S1, S2, R, E, A] =
      environmentWithPure(env => f(env.get))
  }

  /**
   * The `Covariant` instance for `Pure`.
   */
  implicit def PureCovariant[W, S1, S2, R, E]: Covariant[({ type lambda[+A] = Pure[W, S1, S2, R, E, A] })#lambda] =
    new Covariant[({ type lambda[+A] = Pure[W, S1, S2, R, E, A] })#lambda] {
      def map[A, B](f: A => B): Pure[W, S1, S2, R, E, A] => Pure[W, S1, S2, R, E, B] =
        _.map(f)
    }

  /**
   * The `IdentityBoth` instance for `Pure`.
   */
  implicit def PureCovariantIdentityBoth[W, S, R, E]
    : CovariantIdentityBoth[({ type lambda[+A] = Pure[W, S, S, R, E, A] })#lambda] =
    new CovariantIdentityBoth[({ type lambda[+A] = Pure[W, S, S, R, E, A] })#lambda] {
  ...
  /**
   * The `IdentityFlatten` instance for `Pure`.
   */
  implicit def PureIdentityFlatten[W, S, R, E]
    : IdentityFlatten[({ type lambda[+A] = Pure[W, S, S, R, E, A] })#lambda] =
  ...
  implicit final class PureRefineToOrDieOps[W, S1, S2, R, E <: Throwable, A](self: Pure[W, S1, S2, R, E, A]) {

    /**
     * Keeps some of the errors, and `throw` the rest.
     */
    def refineToOrDie[E1 <: E: ClassTag](implicit ev: CanFail[E]): Pure[W, S1, S2, R, E1, A] =
      self.refineOrDie { case e: E1 => e }
  }

  private final case class Succeed[+A](value: A)            extends Pure[Nothing, Any, Nothing, Any, Nothing, A]
  private final case class Fail[+E](error: E)               extends Pure[Nothing, Any, Nothing, Any, E, Nothing]
  private final case class Update[-S1, +S2](run0: S1 => S2) extends Pure[Nothing, S1, S2, Any, Nothing, Unit]
  private final case class FlatMap[+W, -S1, S2, +S3, -R, +E, A, +B](
    value: Pure[W, S1, S2, R, E, A],
    continue: A => Pure[W, S2, S3, R, E, B]
  ) extends Pure[W, S1, S3, R, E, B]
  private final case class FMap[+W, -S1, S2, -R, +E, A, +B](
    value: Pure[W, S1, S2, R, E, A],
    run0: A => B
  ) extends Pure[W, S1, S2, R, E, B]
  private final case class Fold[+W, -S1, S2, +S3, -R, E1, +E2, A, +B](
    value: Pure[W, S1, S2, R, E1, A],
    failure: E1 => Pure[W, S1, S3, R, E2, B],
    success: A => Pure[W, S2, S3, R, E2, B]
  ) extends Pure[W, S1, S3, R, E2, B]
      with Function[A, Pure[W, S2, S3, R, E2, B]] {
  ...
  private object Runner {
    private val free = new ConcurrentLinkedQueue[Runner]()
  ...
  final private class Runner private {
    ...
    private var _environment     = Env.empty
    private var _clearLogOnError = false
    private var _logs            = ChunkBuilder.make[Any]()
    ...
    private def loop[S2, E, A](state: Any, zPure: Erased): Either[E, (S2, A)] = {
      ...
      while (curPure ne null)
        curPure match {
          case flatmap0: Pure.FlatMap[Any, Any, Any, Any, Any, Any, Any, Any] =>
            val nested       = flatmap0.value
            val continuation = flatmap0.continue

            nested match {
              case succeed0: Succeed[Any] =>
                curPure = continuation(succeed0.value)

              case inspect0: Inspect[Any, Any] =>
                curPure = continuation(inspect0.run0(s0))
              ...
              case environment0: Environment[Any, Any, Any, Any, Any, Any] =>
                curPure = continuation(environment0.access(_environment))
          ...
          case succeed0: Succeed[Any]                         =>
            ...
            while (loop)
              stack.pop() match {
                ...
                case other                                         =>
                  loop = false
                  curPure = other.asInstanceOf[Any => Erased](a)
              }
          case fmap0: FMap[Any, Any, Any, Any, Any, Any, Any] =>
            curPure = fmap0.value
            stack.push(fmap0)
          ...
          case fold0: Fold[Any, Any, Any, Any, Any, Any, Any, Any, Any] =>
            ...
            val fold  = if (clear) {
              ...
              Pure.Fold(
                ...
                (error: Any) => {
                  _logs = previousLogs
                  Pure.set(state) *> fold0.failure(error)
                },
                (a: Any) => {
              ...
              Pure.Fold(
                fold0.value,
                Pure.set(state) *> fold0.failure(_: Any),
                fold0.success
              )
            }
          ...
          case provide0: Provide[Any, Any, Any, Any, Any, Any] =>
            ...
            curPure = provide0.continue.foldM(
              e => { _environment = previousEnv; Pure.fail(e) },
              a => { _environment = previousEnv; Pure.succeed(a) }
            )
          ...
          case flag0: ClearLogOnError[Any, Any, Any, Any, Any, Any] =>
            ...
            curPure = flag0.continue.bimap(resetFn, resetFn)

          case fail0: Fail[Any] =>
            curPure = null
            while (curPure eq null)
              stack.pop() match {
                case null                                                     =>
                ...
                case value: Fold[Any, Any, Any, Any, Any, Any, Any, Any, Any] =>
                  curPure = value.failure(fail0.error)
                case _                                                        =>
                  ()
              }
        }
