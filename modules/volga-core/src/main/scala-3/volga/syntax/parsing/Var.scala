...
final class Vars[q <: Quotes & Singleton](using val q: q):
    ...
    def varOf(name: String, t: TypeTree, rhs: Option[Tree] = None): Var[q] =
        import q.reflect.*
        def extractV(tr: TypeRepr): Option[TypeRepr] =
            val deal = tr.dealias
            if deal.show == "<notype>" then None
            else
                deal match
                    case AppliedType(con, List(arg)) if con.typeSymbol == VSym =>
                        Some(arg.dealias)
                    case _ =>
                        deal.baseType(VSym) match
                            case AppliedType(_, List(arg)) => Some(arg.dealias)
                            case _                         => Some(deal)
        val fromTpt = extractV(t.tpe).orElse(extractV(t.tpe.widen).filter(_.show != "<notype>"))
        val fromRhs = rhs.iterator.flatMap {
            case term: Term => extractV(term.tpe).orElse(extractV(term.tpe.widen))
            case _          => None
        }.nextOption()
        TyVar(name, fromTpt.orElse(fromRhs))
    end varOf
